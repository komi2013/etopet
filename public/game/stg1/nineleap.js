...
enchant.nineleap = { assets: ['/public/game/stg1/start.png', '/public/game/stg1/gameover.png'] };
enchant.nineleap.Game = enchant.Class.create(enchant.Game, {
    initialize: function(width, height) {
        ...
        this.addEventListener('load', function() {
            ...
            this.startScene.image = this.assets['/public/game/stg1/start.png'];
            ...
            this.endScene.image = this.assets['/public/game/stg1/gameover.png'];
    ...
    start: function() {
        ...
        if (location.hostname == 'r.jsgames.jp') {
            var submit = function() {
                ...
                location.replace([
                    'https://9leap.net/games/', id, '/result',
    ...
    _setTwitterAssets: function(resBody, requestType, checkError) {
        ...
        if (resBody[0] == undefined) {
            ...
            if (resBody[0].code == 401 && this.requireAuth) {
                window.location.replace('https://9leap.net/api/login?after_login=' + window.location.href);
                return;
...
enchant.twitter.TwitterRequest = enchant.Class.create({
    initialize: function(id, requestType, option, checkError) {
        ...
        var src = 'https://9leap.net/api/twitter/' + requestType + '.json' + callback;
        if (option) {
